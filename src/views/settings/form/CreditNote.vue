...
export default {
  ...
  data() {
    return {
      ...
      form_invoice: CreditNote(this.items, "id_23e", def),
      slider_widht: 5,
      slider_height: 5,
      image_size: {
        width: "100px",
  ...
  methods: {
    saveForm() {
      ...
      axios.post("http://localhost:4000/api/form_setting", data).then((res) => {
    ...
    getFormSetting() {
      axios
        .get("http://localhost:4000/api/get_form_setting", {
          params: {
            institute: instituteId,
            formType: "Credit Note",
          },
        ...
        .then((res) => {
          window.console.log("get setting", res.data.data.Items);
          if (res.data.data.Count > 0) {
            this.invoice_setting = res.data.data.Items["0"].settings;
            this.sk = res.data.data.Items["0"].sk;
            this.pk = res.data.data.Items["0"].pk;
            this.form_invoice = CreditNote(
              this.items,
